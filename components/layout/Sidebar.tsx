...
import {
  LayoutDashboard, Users, LogOut, Building2,
  ChevronRight, Shield
} from 'lucide-react'
...
export default function Sidebar({ role, fullName, email }: SidebarProps) {
  ...
  return (
    <aside style={{
      ...
      <div style={{ padding: '24px 20px 20px', borderBottom: '1px solid #2e2b1e' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '10px', color: '#5c481f', letterSpacing: '0.08em' }}>
              APARTHOTEL
            </div>
          </div>
        </div>
      ...
      <div style={{
        height: '1px',
        background: 'linear-gradient(90deg, #d4ab5a33, transparent)',
      }} />
        ...
        {navItems.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} style={{
              ...
              color: isActive ? roleColor : '#7a6e52',
              textDecoration: 'none',
              ...
              {item.label}
              {isActive && (
                <ChevronRight size={12} style={{ marginLeft: 'auto', color: roleColor, opacity: 0.6 }} />
              )}
            </Link>
          )
        })}
      ...
      <div style={{ padding: '12px', borderTop: '1px solid #2e2b1e' }}>
        ...
        <button
          ...
          onMouseEnter={e => {
            ...
            t.style.borderColor = 'rgba(248,113,113,0.15)'
