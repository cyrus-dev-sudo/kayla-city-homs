...
const STATUS_CONFIG = {
  ...
  cancelled: { color: '#5c481f', label: 'Cancelled', icon: <Circle size={12} /> },
}
  ...
  return (
    <div style={{ padding: '32px', animation: 'fadeIn 0.3s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '28px', fontWeight: 600, color: '#f4e4c1' }}>Tasks</h2>
          <p style={{ fontSize: '13px', color: '#7a6e52', marginTop: '4px' }}>{tasks.filter(t => t.status === 'pending').length} pending · {tasks.filter(t => t.status === 'in_progress').length} in progress</p>
        </div>
        {canAssign && (
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
        {['all', 'pending', 'in_progress', 'completed'].map(s => (
          <button key={s} onClick={() => setFilter(s)} style={{
            ...
            border: `1px solid ${filter === s ? 'rgba(212,171,90,0.3)' : '#2e2b1e'}`,
            borderRadius: '100px', fontSize: '12px', fontWeight: 600,
            color: filter === s ? '#d4ab5a' : '#5c481f', cursor: 'pointer', fontFamily: 'inherit',
          }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {filtered.length === 0 && (
          <div style={{ padding: '48px', textAlign: 'center', background: '#1a1710', border: '1px solid #2e2b1e', borderRadius: '12px' }}>
            <CheckCircle size={36} color="#3a3728" style={{ margin: '0 auto 12px' }} />
            <p style={{ color: '#5c481f' }}>No tasks found</p>
          </div>
        )}
        {filtered.map(task => {
          ...
          return (
            <div key={task.id} style={{ background: '#1a1710', border: '1px solid #2e2b1e', borderRadius: '12px', padding: '20px', display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
              <div style={{ width: '3px', alignSelf: 'stretch', background: priority.color, borderRadius: '2px', flexShrink: 0 }} />
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '8px' }}>
                  <div>
                    <div style={{ fontSize: '14px', fontWeight: 600, color: '#c4b48a', marginBottom: '4px' }}>{task.title}</div>
                    {task.description && <div style={{ fontSize: '12px', color: '#7a6e52' }}>{task.description}</div>}
                  </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', fontSize: '11px', color: '#5c481f', marginBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  ...
                  {canAssign && task.status !== 'cancelled' && <button onClick={() => updateStatus(task.id, 'cancelled')} style={{ padding: '4px 10px', background: 'transparent', border: '1px solid #2e2b1e', borderRadius: '6px', fontSize: '11px', color: '#5c481f', cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>}
                </div>
              </div>
            </div>
          )
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div style={{ background: '#1a1710', border: '1px solid #2e2b1e', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '480px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
              <div>
                <div style={{ height: '2px', background: 'linear-gradient(90deg, #d4ab5a, transparent)', marginBottom: '16px', borderRadius: '2px' }} />
                <h3 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '20px', fontWeight: 600, color: '#f4e4c1' }}>Assign Task</h3>
              </div>
            ...
            <form onSubmit={createTask} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div><label style={LBL}>Task Title *</label><input required value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="Clean Room 201" style={INPUT} onFocus={e => e.target.style.borderColor = '#b8923d'} onBlur={e => e.target.style.borderColor = '#2e2b1e'} /></div>
              <div><label style={LBL}>Description</label><textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="Task details..." rows={2} style={{ ...INPUT, resize: 'vertical' }} onFocus={e => e.target.style.borderColor = '#b8923d'} onBlur={e => e.target.style.borderColor = '#2e2b1e'} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div><label style={LBL}>Assign To</label>
                  <select value={form.assigned_to} onChange={e => setForm(f => ({ ...f, assigned_to: e.target.value }))} style={{ ...INPUT, appearance: 'none' }} onFocus={e => e.target.style.borderColor = '#b8923d'} onBlur={e => e.target.style.borderColor = '#2e2b1e'}>
              ...
              <div><label style={LBL}>Due Date</label><input type="date" value={form.due_date} onChange={e => setForm(f => ({ ...f, due_date: e.target.value }))} style={INPUT} onFocus={e => e.target.style.borderColor = '#b8923d'} onBlur={e => e.target.style.borderColor = '#2e2b1e'} /></div>
              <div style={{ display: 'flex', gap: '10px', marginTop: '8px' }}>
                ...
                <button type="submit" disabled={loading} style={{ flex: 2, padding: '11px', background: 'linear-gradient(135deg, #b8923d, #d4ab5a)', color: '#111008', fontWeight: 700, fontSize: '13px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit' }}>{loading ? 'Assigning...' : 'Assign Task'}</button>
              </div>
            </form>
          </div>
        </div>
