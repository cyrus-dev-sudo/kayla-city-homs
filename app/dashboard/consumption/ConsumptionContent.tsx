    ...
    <div style={{ padding: '32px', animation: 'fadeIn 0.3s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#7a6650', marginTop: '4px' }}>Today: {records.filter(r => r.created_at.startsWith(new Date().toISOString().split('T')[0])).length} records</p>
        </div>
            ...
            <thead>
              <tr style={{ background: '#221b10' }}>
                {['Room', 'Item', 'Type', 'Qty', 'Price', 'Shift', 'Time', 'By'].map(h => (
                  <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: '10px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#7a6650', borderBottom: '1px solid #2e2010' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {records.map(r => {
                const color = TYPE_COLORS[r.item_type] || '#a8702e'
                return (
                  <tr key={r.id} style={{ borderBottom: '1px solid #2e2010' }} onMouseEnter={e => (e.currentTarget.style.background = '#221b10')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                    <td style={{ padding: '12px 14px', fontSize: '13px', fontWeight: 600, color: '#c4ab85' }}>Room {r.rooms?.room_number}</td>
                    <td style={{ padding: '12px 14px', fontSize: '13px', color: '#c4ab85' }}>{r.item_name}</td>
                    ...
                    <td style={{ padding: '12px 14px', fontSize: '13px', color: '#7a6650' }}>×{r.quantity}</td>
                    <td style={{ padding: '12px 14px', fontSize: '13px', color: '#a8702e' }}>GH₵{(r.unit_price * r.quantity).toFixed(2)}</td>
                    <td style={{ padding: '12px 14px', fontSize: '11px', color: '#7a6650', textTransform: 'capitalize' }}>{r.shift}</td>
                    <td style={{ padding: '12px 14px', fontSize: '11px', color: '#7a6650' }}>{new Date(r.created_at).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</td>
                    <td style={{ padding: '12px 14px', fontSize: '11px', color: '#7a6650' }}>{r.recorded_by_profile?.full_name}</td>
                  </tr>
                )
              })}
            </tbody>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <div style={{ background: '#1a160c', border: '1px solid #2e2010', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '440px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
              <div><div style={{ height: '2px', background: 'linear-gradient(90deg, #a8702e, transparent)', marginBottom: '16px', borderRadius: '2px' }} /><h3 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '20px', fontWeight: 600, color: '#f0d3a8' }}>Record Consumption</h3></div>
            ...
            <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div><label style={LBL}>Type</label>
                  <select value={form.item_type} onChange={e => setForm(f => ({ ...f, item_type: e.target.value, item_name: '' }))} style={{ ...INPUT, appearance: 'none' }} onFocus={e => e.target.style.borderColor = '#93602a'} onBlur={e => e.target.style.borderColor = '#2e2010'}>
              ...
              <div><label style={LBL}>Item *</label>
                <select value={form.item_name} onChange={e => setForm(f => ({ ...f, item_name: e.target.value }))} style={{ ...INPUT, appearance: 'none' }} onFocus={e => e.target.style.borderColor = '#93602a'} onBlur={e => e.target.style.borderColor = '#2e2010'}>
                  <option value="">Select item...</option>
                  {ITEMS[form.item_type as keyof typeof ITEMS]?.map(i => <option key={i}>{i}</option>)}
                </select>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                <div><label style={LBL}>Qty</label><input type="number" min="1" value={form.quantity} onChange={e => setForm(f => ({ ...f, quantity: e.target.value }))} style={INPUT} onFocus={e => e.target.style.borderColor = '#93602a'} onBlur={e => e.target.style.borderColor = '#2e2010'} /></div>
                <div><label style={LBL}>Unit Price</label><input type="number" value={form.unit_price} onChange={e => setForm(f => ({ ...f, unit_price: e.target.value }))} placeholder="0.00" style={INPUT} onFocus={e => e.target.style.borderColor = '#93602a'} onBlur={e => e.target.style.borderColor = '#2e2010'} /></div>
                <div><label style={LBL}>Shift</label>
                  <select value={form.shift} onChange={e => setForm(f => ({ ...f, shift: e.target.value }))} style={{ ...INPUT, appearance: 'none' }} onFocus={e => e.target.style.borderColor = '#93602a'} onBlur={e => e.target.style.borderColor = '#2e2010'}>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '8px' }}>
                ...
                <button type="submit" disabled={loading || !form.item_name} style={{ flex: 2, padding: '11px', background: 'linear-gradient(135deg, #93602a, #a8702e)', color: '#111008', fontWeight: 700, fontSize: '13px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontFamily: 'inherit' }}>{loading ? 'Recording...' : 'Record'}</button>
              </div>
            </form>
          </div>
        </div>
      )}
      <style>{`@keyframes fadeIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}} select option{background:#221b10;color:#f0d3a8}`}</style>
    </div>
  )
}
