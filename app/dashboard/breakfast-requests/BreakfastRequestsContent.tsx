  ...
  return (
    <div style={{ padding: '32px', animation: 'fadeIn 0.3s ease' }}>
      <div style={{ marginBottom: '24px' }}>
        <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '28px', fontWeight: 600, color: '#f4e4c1' }}>Breakfast Requests</h2>
        <p style={{ fontSize: '13px', color: '#7a6e52', marginTop: '4px' }}>{todayCount} requests today · {requests.filter(r => r.status === 'pending').length} pending</p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {filtered.length === 0 && (
          <div style={{ padding: '48px', textAlign: 'center', background: '#1a1710', border: '1px solid #2e2b1e', borderRadius: '12px' }}>
            <Coffee size={36} color="#3a3728" style={{ margin: '0 auto 12px' }} />
            <p style={{ color: '#5c481f' }}>No breakfast requests</p>
          </div>
        )}
        {filtered.map(req => {
          ...
          return (
            <div key={req.id} style={{ background: '#1a1710', border: `1px solid ${status.color}25`, borderRadius: '12px', padding: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '12px' }}>
                <div>
                  <div style={{ fontSize: '15px', fontWeight: 700, color: '#f4e4c1' }}>Room {req.room_number}</div>
                  <div style={{ fontSize: '12px', color: '#7a6e52' }}>{req.guest_name} · requested {formatTime(req.requested_at)}</div>
                </div>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '12px' }}>
                {items.length === 0 ? (
                  <span style={{ fontSize: '12px', color: '#3a3728' }}>No items selected</span>
                ) : items.map(item => (
                  <span key={item} style={{ padding: '4px 10px', background: '#221f14', border: '1px solid #2e2b1e', borderRadius: '100px', fontSize: '12px', color: '#c4b48a' }}>{item}</span>
                ))}
              </div>

              {req.special_notes && (
                <div style={{ fontSize: '12px', color: '#7a6e52', marginBottom: '12px', padding: '8px 12px', background: '#221f14', borderRadius: '8px' }}>
                  <strong style={{ color: '#5c481f' }}>Note:</strong> {req.special_notes}
                </div>
              )}
              ...
              {req.status === 'delivered' && req.fulfilled_by_profile && (
                <div style={{ fontSize: '11px', color: '#3a3728' }}>Delivered by {req.fulfilled_by_profile.full_name}</div>
              )}
            </div>
          )
        })}
