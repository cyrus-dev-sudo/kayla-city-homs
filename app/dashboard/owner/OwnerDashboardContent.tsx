...
function KPI({ label, value, icon, accent, href }: { label: string; value: number | string; icon: React.ReactNode; accent: string; href?: string }) {
  const content = (
    <div style={{ background: '#1a1710', border: '1px solid #2e2b1e', borderRadius: '12px', padding: '18px 20px', position: 'relative', overflow: 'hidden', transition: 'border-color 0.2s', cursor: href ? 'pointer' : 'default' }}
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div>
          <div style={{ fontSize: '11px', color: '#5c481f', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '6px' }}>{label}</div>
          <div style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '30px', fontWeight: 600, color: '#f4e4c1', lineHeight: 1 }}>{value}</div>
        </div>
  ...
  return (
    <div style={{ padding: '32px', animation: 'fadeIn 0.3s ease' }}>
      ...
      <div style={{ marginBottom: '32px' }}>
        <p style={{ fontSize: '12px', color: '#5c481f', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '6px' }}>{greeting}</p>
        <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: '32px', fontWeight: 600, color: '#f4e4c1' }}>
      ...
      {/* Occupancy Banner */}
      <div style={{ background: '#1a1710', border: '1px solid #2e2b1e', borderRadius: '12px', padding: '20px 24px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '24px' }}>
        <div style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            <span style={{ fontSize: '12px', color: '#7a6e52', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Hotel Occupancy</span>
            <span style={{ fontSize: '13px', fontWeight: 700, color: '#d4ab5a' }}>{occupancyPct}%</span>
          </div>
          <div style={{ height: '6px', background: '#2e2b1e', borderRadius: '3px', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${occupancyPct}%`, background: 'linear-gradient(90deg, #b8923d, #d4ab5a)', borderRadius: '3px', transition: 'width 0.6s ease' }} />
          </div>
          <div style={{ display: 'flex', gap: '16px', marginTop: '10px' }}>
            {[
              ...
              <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                ...
                <span style={{ fontSize: '11px', color: '#7a6e52' }}>{item.label}: <strong style={{ color: item.color }}>{item.value}</strong></span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '14px', marginBottom: '28px' }}>
        ...
        <KPI label="Total Rooms" value={kpis.totalRooms} icon={<BedDouble size={16} />} accent="#d4ab5a" href="/dashboard/rooms" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '20px' }}>
        <div style={{ background: '#1a1710', border: '1px solid #2e2b1e', borderRadius: '12px', overflow: 'hidden' }}>
          <div style={{ padding: '16px 20px', borderBottom: '1px solid #2e2b1e', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <h3 style={{ fontSize: '14px', fontWeight: 600, color: '#c4b48a' }}>Staff</h3>
              <p style={{ fontSize: '11px', color: '#5c481f', marginTop: '2px' }}>{staff.length} accounts</p>
            </div>
            <Link href="/dashboard/users" style={{ padding: '6px 14px', background: 'rgba(212,171,90,0.1)', border: '1px solid rgba(212,171,90,0.2)', borderRadius: '6px', fontSize: '11px', fontWeight: 600, color: '#d4ab5a', textDecoration: 'none' }}>Manage →</Link>
          </div>
          {staff.length === 0 ? (
            <div style={{ padding: '40px', textAlign: 'center' }}>
              <Users size={32} color="#3a3728" style={{ margin: '0 auto 12px' }} />
              <p style={{ fontSize: '13px', color: '#5c481f' }}>No staff yet</p>
              <Link href="/dashboard/users" style={{ display: 'inline-flex', marginTop: '12px', padding: '8px 16px', background: 'linear-gradient(135deg, #b8923d, #d4ab5a)', color: '#111008', fontWeight: 600, fontSize: '12px', borderRadius: '6px', textDecoration: 'none' }}>Add First Staff</Link>
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr style={{ background: '#221f14' }}>
                {['Name', 'Role', 'Status', 'Joined'].map(h => <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: '10px', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#5c481f', borderBottom: '1px solid #2e2b1e' }}>{h}</th>)}
              </tr></thead>
              <tbody>
                {staff.slice(0, 8).map(member => {
                  ...
                  return (
                    <tr key={member.id} style={{ borderBottom: '1px solid #2e2b1e' }} onMouseEnter={e => (e.currentTarget.style.background = '#221f14')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ fontSize: '13px', fontWeight: 500, color: '#c4b48a' }}>{member.full_name}</div>
                      ...
                      <td style={{ padding: '12px 16px', fontSize: '12px', color: '#5c481f' }}>{new Date(member.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</td>
                    </tr>
                  )
                })}
              </tbody>
        ...
        {/* Role Distribution */}
        <div style={{ background: '#1a1710', border: '1px solid #2e2b1e', borderRadius: '12px', padding: '20px' }}>
          <div style={{ height: '2px', background: 'linear-gradient(90deg, #d4ab5a, transparent)', borderRadius: '2px', marginBottom: '20px' }} />
          <h3 style={{ fontSize: '14px', fontWeight: 600, color: '#c4b48a', marginBottom: '4px' }}>Staff by Role</h3>
            ...
            return (
              <div key={role} style={{ marginBottom: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                  <span style={{ fontSize: '12px', color: '#7a6e52' }}>{ROLE_CONFIG[role].label}</span>
                  <span style={{ fontSize: '12px', fontWeight: 600, color }}>{count}</span>
                </div>
                <div style={{ height: '4px', background: '#2e2b1e', borderRadius: '2px', overflow: 'hidden' }}>
